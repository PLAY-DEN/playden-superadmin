...
const PlaypointUsage: React.FC = () => {
  ...
  const { playpoints, loading, error, meta } = useSelector(
    (state: RootState) => state.playpoints
  );
  ...
  return (
    <div className="bg-white p-8 rounded-lg">
      {!playpoints || playpoints.length === 0 ? (
        <NotFoundPage errorMessage="No PlayPoints found." />
      ) : (
              ...
              {playpoints.map((usage, index) => (
                <tr key={usage.id}>
                  <td className="border-b p-4 text-sm">
                    {(currentPage - 1) * (meta?.per_page || 0) + index + 1}
                  </td>
